...
define('l20n/context', function(require, exports, module) {
  ...
  function Context(id, options) {
    ...
    var _parser = new Parser(options);
    var _compiler = new Compiler(options);
...
define('l20n/parser', function(require, exports, module) {
  ...
  function Parser(options) {
    ...
    if (options.delimiter) {
      START_DELIMITER = options.delimiter.start;
      END_DELIMITER = options.delimiter.end;
    }
    else {
      START_DELIMITER = '{{';
      END_DELIMITER = '}}';
    ...
    function getString(opchar) {
      ...
      return {
        ...
        // if there is no {{ (or whatever delimiter was specified) at all, it's definitely not a complex string.
        // if there's a \{{, it's not complex either, but we will learn that 
        // lazily in parseString
        isNotComplex: str.indexOf(START_DELIMITER) === -1 || undefined
      };
    }
    ...
    function getComplexString() {
       ...
       *   - expression openings: {{ (or whatever delimiter was specified)
       *   - escape chars: \
       * 
      ...
      // unescape \\ \' \" \{{ (or whatever delimiter was specified)
      ...
      // parse expressions
      pos = _source.indexOf(START_DELIMITER);
      while (pos !== -1) {
        ...
        if (_source.charCodeAt(pos - 1) === 92) {
          ...
          pos = _source.indexOf(START_DELIMITER, pos + 2);
          continue;
        }
        if (!complex) {
          body = [];
...
//            _source.charCodeAt(_index + 1) !== 125) {
        ...
        pos = _index + 2;
        bstart = pos;
        pos = _source.indexOf(START_DELIMITER, pos);
      }
