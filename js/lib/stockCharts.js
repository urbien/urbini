define('lib/stockCharts', ['d3', 'dc', 'dcCss', 'crossfilter', 'colorbrewer', 'lib/hammer'], function(d3, dc, dcCss, crossfilter, colorbrewer, Hammer) {  
  ...
  d3 = d3 || window.d3;
  dc = dc || window.dc;
  crossfilter = crossfilter || window.crossfilter;
  colorbrewer = colorbrewer || window.colorbrewer;
...
//  })(Hammer, d3);
  ...
  */
  ...
  //```
	var fromDate = data[data.length-1].dd;
	var toDate = data[0].dd;
	var isMultiYear = toDate - fromDate > 2 * 365 * 24 * 3600 * 1000;
  var gainOrLossChart = dc.pieChart(container.$(".gain-loss-chart")[0]);
  var fluctuationChart = dc.barChart(container.$(".fluctuation-chart")[0]);
  var quarterChart = dc.pieChart(container.$(".quarter-chart")[0]);
  var dayOfWeekChart = dc.rowChart(container.$(".day-of-week-chart")[0]);
  var moveChart = dc.lineChart(container.$(".monthly-move-chart")[0]);
  var volumeChart = dc.barChart(container.$(".monthly-volume-chart")[0]);
  var yearlyBubbleChart;
  if (isMultiYear) {
    var chartHolder = container.$(".yearly-bubble-chart")[0];
    if (chartHolder)
      yearlyBubbleChart = dc.bubbleChart();
  }
  
  var charts = {
    gainOrLossChart: gainOrLossChart,
    fluctuationChart: fluctuationChart,
    quarterChart: quarterChart,
    dayOfWeekChart: dayOfWeekChart,
    moveChart: moveChart,
    volumeChart: volumeChart,
    yearlyBubbleChart: yearlyBubbleChart      
  };
  ...
  //        .yAxisLabel('Index Gain %') // (optional) render a vertical axis lable left of the y axis
          .xAxisLabel('Gain') // (optional) render an axis label below the x axis
          .yAxisLabel('Gain %') // (optional) render a vertical axis lable left of the y axis
          //#### Labels and  Titles
  ...
  //                    "Fluctuation / Index Ratio: " + numberFormat(p.value.fluctuationPercentage) + "%"]
                     ...
                     "Fluctuation / Ratio: " + numberFormat(p.value.fluctuationPercentage) + "%"]
                     .join("\n");
          })
          //#### Customize Axis
...
//        .width(450)
        ...
        .x(d3.time.scale().domain([fromDate, toDate]))
        ...
        .group(indexAvgByMonthGroup, "Monthly Average")
        .valueAccessor(function (d) {
            return d.value.avg;
        })
        // stack additional layers with `.stack`. The first paramenter is a new group.
        // The second parameter is the series name. The third is a value accessor.
        .stack(monthlyMoveGroup, "Monthly Move", function (d) {
            return d.value;
        })
