  ...
  //      tmpl_data = _.extend(tmpl_data, {imgSrc: img});
      ...
      if (U.isA(c, 'CollaborationPoint')) { 
        ...
        if (comments.length > 0) {
          ...
          tmpl_data['v_showCommentsFor'] = encodeURIComponent(U.getLongUri(rUri, snmHint) + '&m_p=' + comments[0] + '&b_p=' + pMeta.backLink);
        }
      }
      if (U.isA(c, 'Votable')) {
        ...
        if (votes.length > 0) {
          ...
          tmpl_data['v_showVotesFor'] = encodeURIComponent(U.getLongUri(rUri, snmHint) + '?m_p=' + votes[0] + '&b_p=' + pMeta.backLink);
        }
      }  
      var nabs = U.getCloneOf(meta, 'ImageResource.nabs');
      if (nabs.length > 0) {
        var pMeta = meta[nabs[0]];
        var uri = encodeURIComponent(U.getLongUri(rUri, snmHint) + '?m_p=' + nabs[0] + '&b_p=' + pMeta.backLink);
