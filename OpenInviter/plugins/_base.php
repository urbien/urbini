...
abstract class openinviter_base
	...
	protected function getElementDOM($string_bulk,$query,$attribute=false)
		...
		$search_val=array();
		$doc=new DOMDocument();
		$this->libxml_use_internal_errors(true);
		if (!empty($string_bulk)) $doc->loadHTML($string_bulk);
		else return false;
		$this->libxml_use_internal_errors(false);
...
//		$headers["Content-Type"]="text/html; charset=utf-8";
		...
		if ($this->settings['transport']=='curl')
			...
			// echo "get result: $result<br />";
			// echo "<br />url: $url, referer: $referer<br />";
			if ($follow)
				{
	...
	protected function post($url,$post_elements,$follow=false,$header=false,$referer=false,$headers=array(),$raw_data=false,$quiet=true)
		{
		// echo "START POST:<br />url: $url<br />follow: $follow<br />, header: $header<br />, raw_data: $raw_data<br />, quiet: $quiet<br />, referer: $referer<br />post_elements: ";
		print_r($post_elements);
		// echo "<br />headers: ";
		print_r($headers);
		// echo "END POST";
//		echo "posting to url: $url, with post_elements ";
//		echo $post_elements['login'];
//		echo ",";
//		echo $post_elements['passw'];
		$flag=false;
		if ($raw_data)
...
//				echo $value;
	...
	public function libxml_use_internal_errors($blah) 
		{
		}
