...
class rambler extends openinviter_base
	...
	public function getMyContacts()
		...
		if ($this->checkResponse("url_contacts",$res))
			...
			{
			// echo "failed get contacts url<br />";
		...
		if (!empty($emailsArray))
			{
			foreach($emailsArray as $key=>$emailValue) 
				{
				$contacts[trim($emailValue)]=array('first_name'=>(isset($namesArray[$key])?trim($namesArray[$key]):false),'email_1'=>trim($emailValue));
				}
			}				
